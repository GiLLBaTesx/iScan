        ...
        composable("scanner_test") {
            ScannerTestScreen(
                ...
                onScanTestSheet = { expectedAnswers ->
                    // For now, just show a message since we haven't implemented test result comparison yet
                    Toast.makeText(
                        context, 
                        "Scanning test sheet with ${expectedAnswers.size} questions. Results will be compared after scan.",
                        Toast.LENGTH_LONG
                    ).show()
