...
class ExamRepository(private val dao: ExamDao) {
    ...
    fun getAllMelcs(): Flow<List<MelcEntity>> {
        return flow {
            // Get all unique subjects and grade levels
            val allMelcs = mutableListOf<MelcEntity>()
            // For now, return sample data - in production this would query all MELCs
            emit(com.examscanner.premium.data.SampleMelcsData.getAllSampleMelcs())
        }
    }
    ...
    suspend fun saveQuestionMelcMappings(examId: Long, mappings: Map<Int, Long>) {
        ...
        // Insert new mappings
        val entities = mappings.map { (questionNum, melcId) ->
    ...
    suspend fun getUnassignedStudents(examId: Long): List<StudentEntity> {
        return dao.getUnassignedStudents(examId).first()
    }
    ...
    suspend fun autoOrganizeSections(examId: Long, folderId: Long) {
        ...
        unassignedStudents.chunked(groupSize).forEachIndexed { index, group ->
            ...
            val section = SectionEntity(
                name = sectionName,
                gradeLevel = group.firstOrNull()?.gradeLevel ?: "Mixed",
                subjectFolderId = folderId
            )
