        ...
        composable("new_exam") {
            NewExamScreen(
                ...
                onCreate = { name, questions, subject, section ->
                    scope.launch {
                        // Simple: just create exam, ViewModel will handle folder
                        val examId = repository.createExam(name, questions, 0, subject, section)
                        
                        // Auto-create subject folder if needed
                        if (subject.isNotEmpty()) {
                            viewModel.createSubjectFolder(subject)
                        }
