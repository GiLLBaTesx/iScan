        ...
        return try {
            ...
            context.getDatabasePath(DB_NAME).let { db ->
                if (db.exists()) {
                    ...
                    try {
                        val dbHelper = android.database.sqlite.SQLiteDatabase.openDatabase(
                            db.absolutePath,
                            null,
            ...
            if (!verifyBackup(backupFile)) {
                backupFile.delete()
                return Result.failure(Exception("Backup verification failed"))
            }
    ...
    suspend fun restoreBackup(context: Context, backupUri: Uri): Result<Boolean> {
        return try {
            ...
            context.getDatabasePath(DB_NAME).let { db ->
                if (db.exists()) {
                    try {
                        ...
                        Log.w(TAG, "Could not close database", e)
                    }
                }
            }
            ...
            if (dbFile.exists()) {
                ...
                val currentBackupFile = File(currentBackupDir, "backup_before_restore_${System.currentTimeMillis()}.db")
