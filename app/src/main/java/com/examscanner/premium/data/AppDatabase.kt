...
interface ExamDao {
    ...
    @Query("SELECT * FROM melcs ORDER BY gradeLevel, subject, quarter, code")
    fun getAllMelcs(): Flow<List<MelcEntity>>
    
    @Query("SELECT * FROM melcs ORDER BY gradeLevel, subject, quarter, code")
    suspend fun getAllMelcsSync(): List<MelcEntity>
    ...
    @Query("SELECT * FROM students WHERE studentId = :studentId AND sectionId = :sectionId LIMIT 1")
    ...
    @Query("SELECT * FROM students WHERE examId = :examId AND (sectionId = 0 OR sectionId IS NULL)")
    fun getUnassignedStudents(examId: Long): Flow<List<StudentEntity>>
    
    @Query("UPDATE students SET sectionId = :sectionId WHERE id = :studentId")
    suspend fun updateStudentSection(studentId: Long, sectionId: Long)
    
    @Query("SELECT * FROM student_answers WHERE studentEntityId IN (SELECT id FROM students WHERE examId = :examId) AND questionNumber = :questionNumber")
    fun getStudentAnswersForQuestion(examId: Long, questionNumber: Int): Flow<List<StudentAnswerEntity>>
    ...
    // ------------------------------------------------------------
    // Task 3.2: Subject/Section recycle-bin symmetry, retention
    // purge sweep, paginated exam reads, and student-record lookup.
    // Mirrors the existing exam recycle-bin queries above.
    // ------------------------------------------------------------
    
    // -- Deleted subject folders (recycle bin view) --
    @Query("SELECT * FROM subject_folders WHERE isDeleted = 1 AND deletedAt > :cutoffTime ORDER BY deletedAt DESC")
    fun getDeletedSubjectFolders(cutoffTime: Long): Flow<List<SubjectFolderEntity>>
    
    // Sync variant for the purge sweep (permanently remove those older than the cutoff).
    @Query("SELECT * FROM subject_folders WHERE isDeleted = 1 AND deletedAt IS NOT NULL AND deletedAt <= :cutoffTime")
    suspend fun getExpiredSubjectFolders(cutoffTime: Long): List<SubjectFolderEntity>
    
    @Query("UPDATE subject_folders SET isDeleted = 0, deletedAt = NULL WHERE id = :folderId")
    suspend fun restoreSubjectFolder(folderId: Long)
    
    @Query("DELETE FROM subject_folders WHERE id = :folderId")
    suspend fun hardDeleteSubjectFolder(folderId: Long)
    
    // -- Deleted sections (recycle bin view) --
    @Query("SELECT * FROM sections WHERE isDeleted = 1 AND deletedAt > :cutoffTime ORDER BY deletedAt DESC")
    fun getDeletedSections(cutoffTime: Long): Flow<List<SectionEntity>>
    
    // Sync variant for the purge sweep.
    @Query("SELECT * FROM sections WHERE isDeleted = 1 AND deletedAt IS NOT NULL AND deletedAt <= :cutoffTime")
    suspend fun getExpiredSections(cutoffTime: Long): List<SectionEntity>
    
    @Query("UPDATE sections SET isDeleted = 0, deletedAt = NULL WHERE id = :sectionId")
    suspend fun restoreSection(sectionId: Long)
    
    @Query("DELETE FROM sections WHERE id = :sectionId")
    suspend fun hardDeleteSection(sectionId: Long)
    
    // Sync variant of getDeletedExams for the purge sweep.
    @Query("SELECT * FROM exams WHERE isDeleted = 1 AND deletedAt IS NOT NULL AND deletedAt <= :cutoffTime")
    suspend fun getExpiredExams(cutoffTime: Long): List<ExamEntity>
    
    // -- Paginated exams by subject folder (Req 19.2) --
    @Query("SELECT * FROM exams WHERE subjectFolderId = :folderId AND isDeleted = 0 ORDER BY createdAt DESC LIMIT :limit OFFSET :offset")
    suspend fun getExamsBySubjectPaged(folderId: Long, limit: Int, offset: Int): List<ExamEntity>
    
    // -- All StudentEntity rows for a given school studentId (String), across exams/sections --
    @Query("SELECT * FROM students WHERE studentId = :studentId ORDER BY scannedAt")
    suspend fun getAllStudentRecords(studentId: String): List<StudentEntity>
    
    // -- A single StudentEntity row by its Long primary key (used by MasteryCalculator
    //    to resolve the school studentId String for cross-exam aggregation) --
    ...
    // ============================================================
    @Query("SELECT * FROM student_enrollments WHERE sectionId = :sectionId ORDER BY enrolledAt")
    fun getEnrollmentsBySection(sectionId: Long): Flow<List<StudentEnrollmentEntity>>
    
    @Query("SELECT * FROM student_enrollments WHERE studentId = :studentId ORDER BY enrolledAt")
    fun getEnrollmentsByStudent(studentId: Long): Flow<List<StudentEnrollmentEntity>>
    
    @Query("SELECT * FROM student_enrollments WHERE studentId = :studentId AND sectionId = :sectionId LIMIT 1")
    suspend fun getEnrollment(studentId: Long, sectionId: Long): StudentEnrollmentEntity?
    
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun enrollStudent(enrollment: StudentEnrollmentEntity): Long
    
    @Query("DELETE FROM student_enrollments WHERE studentId = :studentId AND sectionId = :sectionId")
    suspend fun unenrollStudent(studentId: Long, sectionId: Long)
    
    // ============================================================
    // MELC Coverage (melc_coverage)
    // Read-by-subject/quarter filters through the melcs table since
    // melc_coverage references melcId (MelcEntity holds subject + quarter).
    // ============================================================
    @Query("SELECT * FROM melc_coverage WHERE melcId = :melcId LIMIT 1")
    suspend fun getMelcCoverageByMelc(melcId: Long): MelcCoverageEntity?
    
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertMelcCoverage(coverage: MelcCoverageEntity): Long
    
    /**
     * Upsert coverage status for a MELC. Deletes any existing coverage rows for
     * the same MELC first so a single logical coverage record is maintained,
     * then inserts the new one. Wrapped in a transaction for atomicity.
     */
    @Transaction
    suspend fun upsertMelcCoverage(coverage: MelcCoverageEntity): Long {
        deleteMelcCoverageByMelc(coverage.melcId)
        return insertMelcCoverage(coverage)
    }
    
    @Query("DELETE FROM melc_coverage WHERE melcId = :melcId")
    suspend fun deleteMelcCoverageByMelc(melcId: Long)
    
    @Query("""
        SELECT mc.* FROM melc_coverage mc
        INNER JOIN melcs m ON mc.melcId = m.id
        WHERE m.subject = :subject AND m.quarter = :quarter
        ORDER BY mc.coveredAt DESC
    """)
    fun getCoverageBySubjectAndQuarter(subject: String, quarter: Int): Flow<List<MelcCoverageEntity>>
    
    @Query("SELECT * FROM melc_coverage WHERE subjectId = :subjectId ORDER BY coveredAt DESC")
    fun getCoverageBySubjectFolder(subjectId: Long): Flow<List<MelcCoverageEntity>>
...
@Database(
    entities = [
        ...
        // NET-NEW: entity files already exist in data/, registered here via MIGRATION_5_6
...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_4_5 = object : Migration(4, 5) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // Remove redundant subject and section columns (folders handle organization)
                ...
                database.execSQL("""
                    INSERT INTO exams_new (id, subjectFolderId, sectionId, templateId, name, totalQuestions, qrCode, gradingScale, passingGrade, useNegativeMarking, negativeMarkValue, examDate, timeLimit, allowLateScans, createdAt, isDeleted, deletedAt)
                    SELECT id, subjectFolderId, sectionId, templateId, name, totalQuestions, qrCode, gradingScale, passingGrade, useNegativeMarking, negativeMarkValue, examDate, timeLimit, allowLateScans, createdAt, isDeleted, deletedAt FROM exams
                """)
        ...
        /**
         * v5 -> v6: register the four net-new tables whose entity files already exist
         * (StudentEnrollmentEntity, MelcCoverageEntity, StudentNoteEntity, SyncLogEntity).
         *
         * Purely additive: no existing table is renamed, dropped, or recreated, so all
         * pre-existing v5 data is preserved. Column shapes and indices below match the
         * shipped entity files exactly (data/StudentEnrollmentEntity.kt,
         * MelcCoverageEntity.kt, StudentNoteEntity.kt, SyncLogEntity.kt).
         */
        // Exposed (read-only) for the 5->6 Room migration test. The builder above
        // references this same instance, so the test validates the production migration
        // rather than a duplicate. Kept internal + @VisibleForTesting to avoid widening
        // the public API surface.
        @androidx.annotation.VisibleForTesting
        internal val MIGRATION_5_6 = object : Migration(5, 6) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // student_enrollments: student <-> section join with unique (studentId, sectionId)
                database.execSQL("""
                ...
                // melc_coverage: manual MELC coverage tracking
                database.execSQL("""
                ...
                """)
                database.execSQL("CREATE INDEX IF NOT EXISTS index_melc_coverage_melcId ON melc_coverage (melcId)")
                database.execSQL("CREATE INDEX IF NOT EXISTS index_melc_coverage_subjectId ON melc_coverage (subjectId)")

                // student_notes: teacher notes attached to a student
                database.execSQL("""
                ...
                """)
                database.execSQL("CREATE INDEX IF NOT EXISTS index_student_notes_studentId ON student_notes (studentId)")

                // sync_logs: standalone cloud-sync audit log (no foreign keys)
                database.execSQL("""
