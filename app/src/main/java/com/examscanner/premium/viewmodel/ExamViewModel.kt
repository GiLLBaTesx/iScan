...
data class FolderExamsPagedUiState(
...
class ExamViewModel(private val repository: ExamRepository) : ViewModel() {
    ...
    // ------------------------------------------------------------------
    // Task 14 (Req 19.2): paginated exam-list loading (20 items per page)
    // backed by ExamRepository.getExamsBySubjectPaged. Accumulates pages so
    // the UI can render a growing LazyColumn and call loadNextFolderExamsPage
    // when it reaches the end. Query time is logged inside the repository.
    // ------------------------------------------------------------------

    private val _folderExamsPaged = MutableStateFlow(FolderExamsPagedUiState())
    val folderExamsPaged: StateFlow<FolderExamsPagedUiState> = _folderExamsPaged.asStateFlow()

    /**
     * Start (or restart) paginated loading of a folder's exams, resetting to the
     * first page. Loads [pageSize] items (default 20 per Req 19.2).
     */
    fun startFolderExamsPaging(folderId: Long, pageSize: Int = PAGE_SIZE) {
        _folderExamsPaged.value = FolderExamsPagedUiState(
            folderId = folderId,
            pageSize = pageSize,
            isLoading = true
        )
        loadFolderExamsPage(reset = true)
    }

    /** Load the next page for the folder currently being paged. */
    fun loadNextFolderExamsPage() {
        val current = _folderExamsPaged.value
        if (current.folderId == null || current.endReached || current.isLoading) return
        loadFolderExamsPage(reset = false)
    }

    private fun loadFolderExamsPage(reset: Boolean) {
        val current = _folderExamsPaged.value
        val folderId = current.folderId ?: return
        val offset = if (reset) 0 else current.exams.size
        viewModelScope.launch {
            _folderExamsPaged.value = current.copy(isLoading = true, error = null)
            try {
                val page = repository.getExamsBySubjectPaged(folderId, current.pageSize, offset)
                val accumulated = if (reset) page else current.exams + page
                _folderExamsPaged.value = _folderExamsPaged.value.copy(
                    exams = accumulated,
                    isLoading = false,
                    endReached = page.size < current.pageSize
                )
            } catch (e: Exception) {
                _folderExamsPaged.value = _folderExamsPaged.value.copy(
                    isLoading = false,
    ...
    companion object {
        /** Req 19.2: exam lists load 20 items per page. */
